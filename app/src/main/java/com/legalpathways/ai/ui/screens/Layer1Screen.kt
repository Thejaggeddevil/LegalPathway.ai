        ...
        Column(modifier = Modifier.fillMaxSize().padding(padding).padding(16.dp)) {
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                Column(modifier = Modifier.weight(1f)) {
                    Text("Religion", style = MaterialTheme.typography.labelMedium, color = TextHint)
                    Spacer(Modifier.height(4.dp))
                    DropdownSelector(
                        options  = listOf("Hindu" to "Hindu","Muslim" to "Muslim","Christian" to "Christian","Special Marriage" to "Special Marriage"),
                        selected = religion, onSelect = { religion = it }
                    )
                }
                Column(modifier = Modifier.weight(1f)) {
                    Text("Your Role", style = MaterialTheme.typography.labelMedium, color = TextHint)
                    Spacer(Modifier.height(4.dp))
                    DropdownSelector(
            ...
            when (val s = state) {
                ...
                is UiState.Success -> {
                    ...
                    Column(modifier = Modifier.verticalScroll(rememberScrollState())) {
                        when (activeTab) {
                            0 -> data.eligibility?.let { EligibilitySection(it) }
...
fun EligibilitySection(elig: com.legalpathways.ai.model.EligibilityData) {
    Card(modifier = Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = BgCard),
        border = BorderStroke(1.dp, BorderLight), elevation = CardDefaults.cardElevation(1.dp)) {
        Column(modifier = Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
            Text("${elig.religion} – Eligibility", style = MaterialTheme.typography.titleMedium,
...
fun EvidenceSection(items: List<EvidenceItem>, role: String, label: String) {
    val (accentColor, bgColor) = when (label) {
        "High"   -> Pair(GoldPrimary,   GoldPrimary.copy(alpha = 0.07f))
        "Medium" -> Pair(NavyMid,       NavyMid.copy(alpha = 0.07f))
        else     -> Pair(SlateGray,     SlateGray.copy(alpha = 0.07f))
    }
    if (items.isEmpty()) {
        Card(modifier = Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = BgCard),
            border = BorderStroke(1.dp, BorderLight)) {
            Box(modifier = Modifier.padding(20.dp), contentAlignment = Alignment.Center) {
                Text("No $label evidence items.", style = MaterialTheme.typography.bodySmall, color = TextHint)
            }
        }
        return
    }
    Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
        items.forEach { item ->
            ExpandableCard(
                title = item.type,
                badge = { StatusBadge(item.legalWeight, accentColor) }
            ) {
                Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                    item.description?.let {
                        Text(it, style = MaterialTheme.typography.bodySmall, color = TextSecondary, lineHeight = 20.sp)
                    }
